  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (const row of rows) {
        const thumbnailUrl = await s3Upload(row.thumbFile, "/api/s3/productImgUpload");
      ...
      // keep top-level fields for backward compatibility (first row)
      const topThumb = rowsPayload[0]?.thumbnail_img || "";
      const topSide = rowsPayload[0]?.side_imgs || [];
      ...
      const res = await fetch("/api/product/createProduct", {
        ...
        body: JSON.stringify({
          ...
          thumbnail_img: topThumb,
          side_imgs: topSide,
        }),
      });

      const data = await res.json();
      if (!res.ok || data?.error) throw new Error(data?.message || "Failed to create product");
