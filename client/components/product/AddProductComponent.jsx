...
// /pages/your-path/AddProductComponent.jsx
"use client";
import React, { useEffect, useState } from "react";
import { s3Upload } from "@/lib/s3Upload";
import ErrorBanner from "@/components/product/ErrorBanner";
...
const AddProductComponent = () => {
  ...
  // ====== NEW: Multi-paste helpers for details (Vercel env style) ======
  // Parse lines like: KEY=VALUE / key:value / key<TAB>value
  ...
  // Handle multi-paste into Product Details (key/value inputs)
  const handleDetailsPaste = (e, idx, targetField /* 'key' | 'value' */) => {
    const text = e.clipboardData?.getData("text") || "";
    const looksStructured = /\r?\n/.test(text) || /[=\t:]/.test(text);
    if (!looksStructured) return; // allow normal paste
    ...
    setDetails((prev) => {
      ...
      // Apply first pair into current row
      const first = pairs[0];
      const cur = { ...next[idx] };

      if (targetField === "key" && pairs.length === 1 && first.value === "" && cur.value) {
        cur.key = first.key;
      } else if (targetField === "value" && pairs.length === 1 && first.key === "" && cur.key) {
        cur.value = first.value;
      } else {
        if (first.key !== undefined) cur.key = first.key;
        if (first.value !== undefined) cur.value = first.value;
      }
      next[idx] = cur;

      // Insert remaining lines as new rows below
      if (pairs.length > 1) {
        const rest = pairs.slice(1).map((p) => ({ key: p.key, value: p.value }));
                  ...
                  <div key={i} className="grid grid-cols-12 gap-3 p-4">
                    ...
                    <div className="col-span-12 sm:col-span-5">
                      <input
                        ...
                        onPaste={(e) => handleDetailsPaste(e, i, "key")}
                        className="w-full rounded-lg border border-gray-300 px-3 py-2 outline-none focus:border-gray-500"
                      />
                    </div>
                    ...
                    <div className="col-span-12 sm:col-span-5">
                      <input
                        ...
                        onPaste={(e) => handleDetailsPaste(e, i, "value")}
                        className="w-full rounded-lg border border-gray-300 px-3 py-2 outline-none focus:border-gray-500"
                      />
                    </div>
